...
"""
    v_reality, v_tls = ClashParser().parse_result(yaml_text).nodes
...
proxies:
  - name: h2-ech
    ...
    obfs-password: op
"""
    h2n = ClashParser().parse_result(yaml_text).nodes[0]
...
proxies:
  ...
  - name: http-headers
    ...
    tls: false
"""
    s_tls, http_h = ClashParser().parse_result(yaml_text).nodes
    assert (
...
"""
    wg = ClashParser().parse_result(yaml_text).nodes[0]
