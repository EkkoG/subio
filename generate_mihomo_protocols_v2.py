...
from subio2.models.node import (
    Proxy, NodeType,
    ShadowsocksProtocol, VmessProtocol, VlessProtocol, TrojanProtocol,
    HysteriaProtocol, Hysteria2Protocol, HttpProtocol, Socks5Protocol,
    WireGuardProtocol, TuicProtocol, SSHProtocol, SnellProtocol,
    MieruProtocol, AnyTLSProtocol,
    BasicAuth, TLSConfig, RealityConfig, Transport, WebSocketTransport,
    GRPCTransport, HTTPTransport, HTTP2Transport, QUICTransport,
    SmuxConfig, ECHConfig
)
...
def generate_shadowsocks_proxies() -> List[Proxy]:
    ...
    plugins = [
        ...
        ('v2ray-plugin', {'mode': 'websocket', 'tls': True, 'host': 'example.com', 'path': '/ws'}),
        ('shadow-tls', {'version': 3, 'host': 'example.com', 'password': 'shadow-tls-password'}),
        ('restls', {'version-hint': 'tls13', 'host': 'example.com', 'password': 'restls-password'})
    ]
...
def main():
    ...
    config = {
        ...
        'proxy-groups': [
            ...
            },
            {
                'name': 'Shadowsocks',
                'type': 'select',
                'proxies': [p.name for p in all_proxies if isinstance(p.protocol, ShadowsocksProtocol)]
            },
            {
                'name': 'VMess',
                'type': 'select',
                'proxies': [p.name for p in all_proxies if isinstance(p.protocol, VmessProtocol)]
            },
            {
                'name': 'VLESS',
                'type': 'select',
                'proxies': [p.name for p in all_proxies if isinstance(p.protocol, VlessProtocol)]
            },
            {
                'name': 'Trojan',
                'type': 'select',
                'proxies': [p.name for p in all_proxies if isinstance(p.protocol, TrojanProtocol)]
            },
            {
                'name': 'Hysteria',
                'type': 'select',
                'proxies': [p.name for p in all_proxies if isinstance(p.protocol, (HysteriaProtocol, Hysteria2Protocol))]
            },
            {
                'name': 'WireGuard',
                'type': 'select',
                'proxies': [p.name for p in all_proxies if isinstance(p.protocol, WireGuardProtocol)]
            },
            {
                'name': 'TUIC',
                'type': 'select',
                'proxies': [p.name for p in all_proxies if isinstance(p.protocol, TuicProtocol)]
            },
            {
                'name': 'SSH',
                'type': 'select',
                'proxies': [p.name for p in all_proxies if isinstance(p.protocol, SSHProtocol)]
            },
            {
                'name': 'Snell',
                'type': 'select',
                'proxies': [p.name for p in all_proxies if isinstance(p.protocol, SnellProtocol)]
            },
            {
                'name': 'Mieru',
                'type': 'select',
                'proxies': [p.name for p in all_proxies if isinstance(p.protocol, MieruProtocol)]
            },
            {
                'name': 'AnyTLS',
                'type': 'select',
                'proxies': [p.name for p in all_proxies if isinstance(p.protocol, AnyTLSProtocol)]
            },
            {
                'name': 'HTTP/SOCKS',
                'type': 'select',
                'proxies': [p.name for p in all_proxies if isinstance(p.protocol, (HttpProtocol, Socks5Protocol))]
            },
            {
    ...
    print(f"Generated mihomo_all_protocols_v2.yaml with {len(all_proxies)} proxy configurations")
