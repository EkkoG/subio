...
from .core.registry import parser_registry, renderer_registry, uploader_registry
from .models import Node
from .models.config import Provider, Artifact
...
class SubIO2:
        ...
        for provider in self.config.providers:
            ...
            try:
                ...
                if provider.privacy_endpoint:
                    nodes = self._apply_privacy_endpoint(
                        nodes, provider.privacy_endpoint
                    )
    ...
    def _load_provider_nodes(self, provider: Provider) -> List[Node]:
        ...
        if provider.file:
            ...
            file_path = (
                provider_dir / provider.file
                if provider_dir.exists()
                else self.work_dir / provider.file
            )
    ...
    def _apply_filters(self, nodes: List[Node], artifact: Artifact) -> List[Node]:
        ...
        filters = (
            ...
            if artifact.filters
                ...
                {
                    "include": self.config.filter.include,
                    "exclude": self.config.filter.exclude,
                }
