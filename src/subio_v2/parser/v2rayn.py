...
class V2RayNParser(BaseParser):
    ...
    def _parse_vmess(self, line: str) -> VmessNode | None:
        try:
            ...
            transport = TransportSettings(
                ...
                path=data.get("path"),
                headers={"Host": data.get("host")} if data.get("host") else None # simplified
            )
    ...
    def _parse_ss(self, line: str) -> ShadowsocksNode | None:
        try:
            ...
            if not url.hostname:
                ...
                decoded = base64.b64decode(url.netloc + "=" * ((4 - len(url.netloc) % 4) % 4)).decode('utf-8')
                ...
                if url.username and ":" not in url.username and not url.password:
                     ...
                     decoded_auth = base64.b64decode(url.username + "=" * ((4 - len(url.username) % 4) % 4)).decode('utf-8')
    ...
    def _parse_vless(self, line: str) -> VlessNode | None:
         try:
            ...
            transport = TransportSettings(
                network=Network(type_net) if type_net in [n.value for n in Network] else Network.TCP,
            ...
            tls = TLSSettings(
                ...
                reality_opts={"public-key": q.get("pbk", [""])[0], "short-id": q.get("sid", [""])[0]} if security == "reality" else None
            )
