    ...
    for k, v in config["Proxy"].items():
        ...
        if node["type"] in ["vmess", "trojan"]:
            if node["ws"]:
                ...
                node["ws-headers"] = (
                    parse_headers(node["ws-headers"]) if "ws-headers" in node else {}
                )
                node.pop("ws", None)
        if node["type"] == "ss":
        ...
        if node["type"] == "wireguard":
            ...
            if "peer" in node:
                ...
                for peer_str in peers_str:
                    peer_dict = {}
                    # allowed-ips[^=]*=\s*([\s\S]*?)\s*=
                    allowed_ips_re = re.findall(
                        r"allowed-ips[^=]*=\s*([\s\S]*?)\s*=", peer_str
                    )
                    if len(allowed_ips_re) == 0:
                        # allowed-ips[^=]*=\s*([\s\S]*)
                        allowed_ips_re = re.findall(
                            r"allowed-ips[^=]*=\s*([\s\S]*)", peer_str
                        )
                        peer_dict["allowed-ips"] = [allowed_ips_re[0]]
                        peer_str = peer_str.replace(
                            ",allowed-ips=" + allowed_ips_re[0], ""
                        )
                    else:
