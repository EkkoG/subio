...
def validation(nodes, dest, validate_map):
    def validator(node):
        ...
        def get_map_value(key, field, default_value):
            return (
                validate_map.get(node_type, {})["map"]
                .get(field, {})
                .get(dest, {})
        ...
        def get_protocol_value(key, default_value):
            return (
                validate_map.get(node_type, {})["protocol"]
                .get(dest, {})
                .get(key, default_value)
            )
        ...
        if node_type not in validate_map:
            log.logger.warning(
                f"目标平台 {dest} 不支持协议 {node_type}，忽略 {node['name']}"
            )
            return False
        ...
        for k, v in node.items():
            if get_map_value("policy", k, None) == "unsupport":
                log.logger.warning(
                    f"目标平台 {dest} 不支持配置 {node_type} 的 {k} 字段，忽略 {node['name']}"
                )
                return False
            ...
            def value_allowed(allow_values, v):
                if len(allow_values) > 0 and (
                    v not in allow_values and str(v) not in allow_values
                ):
                    log.logger.warning(
                        f"目标平台 {dest} 不支持 {node_type} 的 {k} 字段的值为 {v}，忽略 {node['name']}"
                    )
                    return False
                return True
